...
src/
|-- renderer/
|   |-- App.tsx                 # React entrypoint; capture mode vs app shell
|   |-- app/
|   |   |-- OverlayApp.tsx      # Wires topbar, active view, and controller hooks
|   |   |-- AppTopbar.tsx       # Navigation, language, update, and save controls
|   |   |-- useAppState.ts      # Renderer state, refs, and derived preview values
|   |   |-- useOverlayController.ts # Composes feature hooks for the shell
|   |   |-- useAppLifecycle.ts  # Startup load, updater status, IPC events, screenshot hooks
|   |   |-- useCsvActions.ts    # CSV selection, loading, preview sample cache
|   |   |-- useLayoutCommands.ts # Widget add/copy/delete/reset and layout edits
|   |   |-- useLayoutKeyboard.ts # Layout keyboard shortcuts and nudging
|   |   |-- usePreviewInteraction.ts # Preview drag, resize, pan pointer handlers
|   |   |-- usePreviewViewport.ts # Preview zoom controls
|   |   |-- useExportActions.ts  # ffmpeg discovery/download and render actions
|   |   |-- useRadioInstallActions.ts # Radio discovery, log listing, script install
|   |   |-- preview*.ts         # Geometry, bounds, resize, and commit helpers
|   |   `-- types.ts            # App-local view/mode types and clone helpers
|   |-- views/
|   |   |-- SourceView.tsx      # CSV/manual-radio source selection
|   |   |-- LayoutView.tsx      # Layout screen wrapper
|   |   |-- layout/             # Layout preview and inspector sub-sections
|   |   |-- ExportView.tsx      # Render and ffmpeg settings
|   |   `-- InstallView.tsx     # Lua logger script installation
|   |-- components/             # Reusable renderer widgets
|   |-- styles.css              # Dark theme styles
|   |-- i18n.ts                 # i18next initialisation and language persistence
|   `-- locales/                # Translation JSON files
|-- main/
|   |-- main.ts                 # Electron window, IPC routing, file dialogs
|   |-- nativeApi.ts            # Backend CSV, rendering, radio, ffmpeg commands
|   |-- frameRenderer.ts        # @napi-rs/canvas wrapper
|   |-- screenshots.ts          # Headless screenshot capture
|   `-- updater.ts              # electron-updater integration
|-- preload/preload.ts          # contextBridge overlayApi and updaterApi
`-- shared/
    |-- types.ts                # Types shared across renderer/main/preload
    |-- util.ts                 # Shared layout, numeric, and widget helpers
    `-- widgetDraw.ts           # Canvas widget drawing for preview and export
scripts/
`-- gen_example_csv.js          # Generates docs/example_session.csv
docs/
|-- example_session.csv         # 30-second demo session for screenshots/testing
